    ...
    with col_btn1:
        if st.button("🚀 Emitir y Generar Remito", type="primary", use_container_width=True):
            ...
            with st.spinner("Generando comprobante PDF, actualizando trazabilidad e inventario..."):
                ...
                # El remito ya queda guardado aunque Drive tenga una falla temporal.
                pdf_link = db.subir_archivo_a_drive(pdf_bytes, f"{nro_remito}.pdf")
                if pdf_link:
                    remito_header["Link_PDF"] = pdf_link
                    if not db.actualizar_link_pdf(nro_remito, pdf_link):
                        st.warning("Remito guardado, pero no se pudo registrar el enlace del PDF.")
                else:
                    st.warning(f"Remito guardado, pero el PDF no pudo subirse a Drive: {getattr(db, 'last_error', 'error no especificado')}")
